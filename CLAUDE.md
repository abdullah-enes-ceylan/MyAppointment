...
| "Yaklaşan randevu" tanımı — tek kaynak | **(2026-09-03)** Bulunan gerçek tutarsızlık: `AppointmentService.getUpcomingCustomerAppointments` (`/me/upcoming`) sadece `appointmentDate > now` filtreliyordu, statüye hiç bakmıyordu — gelecek tarihli bir `CANCELLED` randevu bu listede görünmeye devam ediyordu. `ProfileStatsService`'teki profil sayacı ise zaten doğru şekilde sadece `PENDING`/`APPROVED`'ı sayıyordu — aynı isimle anılan iki şey farklı sonuç veriyordu. Düzeltme: `AppointmentStatus.ACTIVE_STATUSES` (enum üzerinde tek `static final List`, `PENDING`+`APPROVED`) eklendi, `getUpcomingCustomerAppointments`, `getUpcomingBusinessAppointments` (işletme tarafındaki aynı sınıf hata, hiçbir ekranda kullanılmadığı için canlıda hiç fark edilmemişti) ve `ProfileStatsService` ÜÇÜ DE artık aynı sabitten okuyor. Not: `/me/upcoming` ve `/business/{id}/upcoming` bugün frontend'de HİÇ ÇAĞRILMIYOR (`MyAppointmentsPage` `/me`'yi statü ayrımı yapmadan çekip tarihe göre client'ta grupluyor; `ApprovedTab` de `/business/{id}/upcoming` yerine bilerek `/business/{id}` + client'ta `APPROVED` filtresi kullanıyor — gerekçesi kod içi yorumda: NO_SHOW işaretlemesi randevu SAATİ GEÇTİKTEN SONRA yapılır, tarih filtresi bunu gizlerdi). Canlı doğrulandı: gelecek tarihli 1 `APPROVED` + 1 `CANCELLED` randevulu bir müşteride düzeltmeden önce `/me/upcoming` 2, profil sayacı 1 dönüyordu; düzeltmeden sonra üçü de (`/me/upcoming`, `/business/{id}/upcoming`, profil sayacı) 1'de birleşti. Bkz. `UpcomingAppointmentsConsistencyTest`. `AppointmentService`/`AccountDeletionService` içindeki AYRI `List.of(PENDING, APPROVED)` kopyaları (çakışma kontrolü, hesap silme iptal adayları) BİLEREK bu sabite taşınmadı — bugün aynı değerleri taşısalar da kavramsal olarak farklı kurallar (biri "slot dolu mu", biri "silme talebinde iptal edilecek randevu"), aynı sabite bağlamak ileride biri değişince diğerini sessizce etkiler. |
