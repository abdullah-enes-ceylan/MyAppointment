...
Verilen kararlar/gerekçeler, tuzaklar, ortam bilgisi ve bilinen sorunlar için: [NOTLAR.md](NOTLAR.md)
...
**Frontend:** React 19 + Vite + Tailwind CSS 4. **Açık tema, lacivert (`#0e2439`) marka rengi**
...
| Randevu istek mi, direkt mi | **(2026-08-28)** Varsayılan **istek modu** (İstek Kutusu) kalıyor — beta'da asıl risk benimsenme, esnafa "sen onaylamadan hiçbir şey olmaz" diyebilmek önemli. **(2026-09-03) İşletme başına "otomatik onay" anahtarı eklendi:** `Business.autoApprove` (V16 migration, varsayılan `false` — mevcut hiçbir işletmenin davranışı sessizce değişmedi), `InfoTab`'de bir toggle olarak düzenlenebiliyor. Açıksa `AppointmentService.createAppointment` talebi `PENDING` yerine doğrudan `APPROVED` olarak dünyaya getiriyor — İstek Kutusu'nu hiç görmeden "Onaylananlar"a düşüyor; çakışma/slot kontrolü, randevu ufku gibi DİĞER hiçbir kural atlanmıyor, SADECE başlangıç durumu değişiyor. Canlı doğrulandı (gerçek panelde toggle açılıp kaydedildi, gerçek bir randevu talebi atılıp doğrudan `APPROVED` döndüğü ve İstek Kutusu'na hiç düşmediği görüldü). **Kenar bulgusu, bilinçli olarak ele alınmadı:** açık talep sınırı (`max-open-requests-per-business`) SADECE `PENDING` sayıyor — otomatik onaylı bir işletmede bu sayaç hep 0 kalır, yani "aynı işletmenin takvimini doldurma" sınırı bu işletmeler için fiilen devre dışı. Bu, otomatik onayın kendi doğal sonucu (üye olma isteğine dayalı, businessın kendi tercihiyle açtığı bir özellik) — ayrı bir sınır eklenmedi, gerçek kötüye kullanım şikayeti gelirse ele alınır. |
...
| Randevu alma payı (minimum lead time) | **(2026-09-04)** `getAvailableTimeSlots` "bugün" için şu andan (+ `AppointmentPolicyProperties.minimumBookingLeadTime`, varsayılan **15 dakika**) önceki saatleri artık göstermiyor — eskiden `Clock`'a hiç bakmıyordu, sabahın erken saatleri gün boyunca "müsait" görünmeye devam ediyordu. `createAppointment` **AYNI kaynaktan** (`AppointmentExpiryPolicy.getMinimumBookingLeadTime()`) okuyarak aynı eşiği uyguluyor — liste ile oluşturma arasında kural iki ayrı yerde tanımlı DEĞİL, biri değişip diğeri unutulursa sessizce ayrışamaz. Değer `AvailabilityCalculator`'ın ızgara adımıyla (varsayılan 15dk) bilerek aynı büyüklükte. Karşılaştırmalar `LocalDateTime` üzerinden (çıplak `LocalTime` değil) — `LocalTime.plusMinutes` gece yarısını sarardı (23:50+15dk→00:05), bu da sabahın erken saatlerini yanlışlıkla "henüz gelmedi" sanmaya yol açardı. Sabit `Clock`'lu testlerle kanıtlandı (`AppointmentServiceAvailableSlotsTest`, `AppointmentServiceMinimumLeadTimeTest`). |
...
| "Yaklaşan randevu" tanımı — tek kaynak, iki ölü uç silindi | **(2026-09-03)** Bulunan gerçek tutarsızlık: `AppointmentService.getUpcomingCustomerAppointments` (`GET /me/upcoming`) sadece `appointmentDate > now` filtreliyordu, statüye hiç bakmıyordu — gelecek tarihli bir `CANCELLED` randevu bu listede görünmeye devam ediyordu. `ProfileStatsService`'teki profil sayacı ise zaten doğru şekilde sadece `PENDING`/`APPROVED`'ı sayıyordu. `getUpcomingBusinessAppointments` (`GET /business/{id}/upcoming`) aynı sınıf hatayı taşıyordu. Canlı doğrulandı (gelecek tarihli 1 `APPROVED` + 1 `CANCELLED` randevulu müşteride düzeltmeden önce `/me/upcoming` 2, profil sayacı 1 dönüyordu), `AppointmentStatus.ACTIVE_STATUSES` (`PENDING`+`APPROVED`) eklenip düzeltildi. Sonra fark edildi: **her iki uç da frontend'de hiç çağrılmıyordu** — `MyAppointmentsPage` `/me`'yi statü ayrımı yapmadan çekip tarihe göre client'ta grupluyor, `ApprovedTab` da `/business/{id}/upcoming` yerine bilerek `/business/{id}` + client'ta `APPROVED` filtresi kullanıyor (NO_SHOW işaretlemesi randevu saati geçtikten SONRA yapılır, tarih filtresi bunu gizlerdi). Kullanılmayan uç = sınanmayan sözleşme + gereksiz saldırı yüzeyi, bu yüzden **her ikisi de silindi** (`AppointmentController`, `AppointmentService.getUpcomingCustomerAppointments`/`getUpcomingBusinessAppointments`, ilgili repository sorguları, `UpcomingAppointmentsConsistencyTest`). `AppointmentStatus.ACTIVE_STATUSES` KALDI — `ProfileStatsService` hâlâ ondan okuyor, tek kaynak olması hâlâ doğru. **Bir mobil istemci ileride "hazır filtrelenmiş yaklaşan liste" ucuna ihtiyaç duyarsa** yeniden yazılır — eski implementasyon git geçmişinde (commit `4a312fd` ve öncesi). `AppointmentService`/`AccountDeletionService` içindeki AYRI `List.of(PENDING, APPROVED)` kopyaları (çakışma kontrolü, hesap silme iptal adayları) BİLEREK `ACTIVE_STATUSES`'a taşınmadı — bugün aynı değerleri taşısalar da kavramsal olarak farklı kurallar, aynı sabite bağlamak ileride biri değişince diğerini sessizce etkiler. |
| İşletme kapak fotoğrafı depolama | **(2026-08-30)** Yerel diske yazılıyor (`BusinessPhotoStorage` arayüzü arkasında, bkz. `LocalDiskBusinessPhotoStorage`). **Bilinçli kabul edilen risk:** deploy ortamının dosya sistemi kalıcı olmayan bir platform olursa (ör. bazı PaaS'lerde ephemeral disk) tüm işletme fotoğrafları sessizce kaybolur — uygulama hata vermez, `photo_key` DB'de dururken dosyalar diskten gider, kapak görseli aniden gradyana döner. Arayüz sayesinde ucuz bir karar: S3/R2'ye geçiş tek bir implementasyon + bean değişikliği. **Deploy platformu seçilirken bu göz önünde bulundurulmalı** (kalıcı disk garantisi yoksa S3/R2'ye geçmeden prod'a alınmamalı). **(2026-09-01 güncelleme)** docker-compose'daki depolama **bind mount olarak başlayıp named volume'a çevrildi** — bind mount'ta taze bir Linux sunucuda host klasörü Docker tarafından `root:root` oluşturulup `appuser` (UID 1000) yazamıyordu; bu, Postgres 5432 yanılgısıyla BİREBİR AYNI desende Windows/Docker Desktop'ta hiç görünmüyordu (host izinleri container'a gerçek yansımıyor). Named volume, image'da zaten `chown`'lanmış mount yolundan sahipliği otomatik devraldığı için (canlı doğrulandı) bu sınıf hatayı tamamen ortadan kaldırıyor — bedeli, veriye artık `cd`/`rsync` ile değil bir yardımcı container üzerinden erişilmesi. İçerik doğrulaması (`BusinessPhotoService`) istemcinin Content-Type/uzantı iddiasına hiç bakmıyor, `ImageIO`'nun kendi baytlara baktığı magic-byte kontrolü + her yüklemenin JPEG'e yeniden kodlanması sayesinde SVG/HTML tabanlı stored-XSS zaten kapalı — gerçek `<script>` içeren bir SVG hem kendi uzantısıyla hem `.jpg` kılığında canlı denenip ikisi de reddedildi. **(2026-09-05 güncelleme — bu satır artık KISMEN GEÇERSİZ, dikkat.)** Depolama `app.business-photo.storage-provider` (`LOCAL`/`R2`, varsayılan `LOCAL`) config-flag'ine bağlandı; **prod'da `R2` seçilir** (`R2BusinessPhotoStorage`, Cloudflare R2, `cdn.randevumweb.com` custom domain, `Cache-Control: immutable`) — dev/test ortamı hâlâ yerel disk kullanır, o yüzden yukarıdaki kalıcı-disk riski SADECE `storage-provider=local` iken geçerli, artık prod'un kaderi değil. Uçtan uca canlı doğrulandı (bkz. NOTLAR.md). Ayrıca **artık tek "kapak fotoğrafı" değil**, `BusinessPhoto` entity'siyle işletme başına en fazla 5 fotoğraflık bir galeri (V17, ROADMAP 3.17) — ilk fotoğraf otomatik kapak. |
...
| Cloudflare proxy — beta boyunca kapalı (DNS only) | **(2026-09-03)** Domain (`randevumweb.com`) Cloudflare'den alındı. Proxy (turuncu bulut) İKİ ayrı sebeple kapalı tutuluyor: (1) ZORUNLU — açıkken Let's Encrypt'in HTTP-01 doğrulaması (RUNBOOK A8/A10) sunucuya doğrudan ulaşamaz, Caddy hiç sertifika alamaz; (2) bilinçli devam kararı — proxy açık olsaydı gerçek istemci IP'si `X-Forwarded-For` yerine `CF-Connecting-IP`'de gelirdi, `InMemoryRateLimiter` (Faz 3.5) bunu okumuyor, yani rate limiting sessizce bozulurdu. İleride proxy açılırsa Cloudflare SSL/TLS modu **Full (strict)** olmalı (Flexible, Caddy'nin HTTPS'ini Cloudflare-sunucu bacağında düz HTTP'ye çevirir) VE rate limiter `CF-Connecting-IP`'yi okuyacak şekilde güncellenmeli — ikisi de bugün yapılmadı. Detay/prosedür: ROADMAP.md Faz 3.8 madde 5, RUNBOOK.md A3. |
| Caddy syntax/smoke testi — DNS yayılmadan asla prod ACME CA'ya istek gitmez | **(2026-09-03)** Bulunan gerçek hata: DNS henüz `randevumweb.com`'a işaret etmiyorken `Caddyfile`'ı syntax kontrolü için `caddy run` ile ayağa kaldırıldı, Caddy PROD Let's Encrypt'e karşı gerçek bir hesap açıp gerçek bir authorization denemesi yaptı (DNS yok diye başarısız oldu ama deneme yine de sayıldı — saatte-5-hata bütçesinden birini gereksiz yere tüketti). RUNBOOK.md A3.1'e iki güvenli yöntem eklendi: `auto_https off` (sıfır ağ isteği, salt syntax) veya `acme_ca` staging'e sabitli (gerçek ACME akışını zararsız CA'ya karşı dener) — ikisi de gerçekten çalıştırılıp doğrulandı. Aynı gerekçeyle A8'in "neden staging" açıklaması da genişletildi: deploy günü doğası gereği dene/düzelt/tekrar-dene döngüsü, bu döngü prod CA'ya karşı çalışırsa gerçek deploy'dan ÖNCE bütçe tükenir. |
...
| CI eklendi, `main` artık branch protection'lı — DOĞRUDAN PUSH ARTIK YOK | **(2026-09-03)** `.github/workflows/ci.yml` (backend testleri + frontend typecheck/lint/boundary/build + `docker compose build`, üçü paralel job) eklendi, gerçek GitHub runner'ında çalıştırılıp yeşil olduğu doğrulandı. `main`'e branch protection kondu: 3 job da required status check + force-push/silme kapalı. **Canlı denendi ve kanıtlandı:** "status check zorunlu" + "PR zorunlu değil" kombinasyonu GitHub'da mümkün değil — taze bir commit'in push ANINDA zaten bir check sonucu olması bekleniyor (check'ler push'tan SONRA çalışır), yani bu kural PR şartı koymasan bile HER doğrudan push'u reddediyor (boş bir test commit'iyle `! [remote rejected] main -> main (protected branch hook declined)` alındı). Kullanıcı bunu bilerek kabul etti: **artık `main`'e asla doğrudan push yok** — her değişiklik önce bir branch'e push edilip PR açılacak (review ZORUNLU DEĞİL, sadece CI'ın o commit için çalışıp yeşil dönmesi gerekiyor), sonra merge edilecek. Bu, bu dosyadaki "Git Safety Protocol" davranışını DEĞİŞTİRİYOR — bir sonraki oturum `git push` ile main'e doğrudan yazmayı denemesin, reddedilir. Ayrıca: Dependabot haftalık açık (Maven+npm+GitHub Actions, `.github/dependabot.yml`), ama `vulnerability-alerts` (güvenlik uyarıları) HALA KAPALI — ayrı bir onay bekliyor. `AccountDeletionResponseFieldsTest`'te CI'ın ilk gerçek çalıştırmasında yakalanan kararsız (flaky) bir test de aynı turda düzeltildi (bkz. commit `3ceab5e`) — sebebi yarış koşulu değil, `LocalDateTime.toString()` ile Jackson'ın ISO serileştiricisinin aynı ânı bazen farklı yazdırması, sabit (nanosaniyesi≠0, saniyesi≠0) bir `Clock` enjekte edilerek yapısal olarak imkansız hale getirildi. |
...
**(2026-09-05) 3.15 (R2'ye taşıma), 3.16 (panel yeniden tasarımı) ve 3.17 (çoklu işletme
fotoğrafı) tamamlandı, hepsi main'de** — sırasıyla işletme fotoğrafları artık prod'da
Cloudflare R2'de (uçtan uca canlı doğrulandı), panel sol dikey menü + açık tema kabuğa
geçti (sekme içerikleri hâlâ eski tema, bilerek sıradaki adım), işletme başına en fazla
5 fotoğraflık gerçek bir galeri var — backend (`BusinessPhoto`, V17), panel (`GalleryTab`)
ve müşteri tarafı (`BusinessDetailPage`'teki kaydırmalı carousel) PR #35 ile merge edildi.
...
- İşletme fotoğrafları: yerel diskten Cloudflare R2'ye taşındı (ROADMAP 3.15, uçtan uca
  ...
  (`BusinessPhoto` entity, V17) — backend, panel (`GalleryTab`) ve müşteri tarafı
  (`BusinessDetailPage`'teki kaydırmalı carousel) canlı doğrulandı ve main'de (PR #35)
- İşletme paneli yeniden tasarımı: sol dikey menü + açık tema kabuk (ROADMAP 3.16) — sekme
  İÇERİKLERİ (İstek Kutusu/Onaylananlar/Hizmetler/Personel/Çalışma Saatleri) hâlâ eski koyu
  temada, kademeli geçiş bilerek sıradaki adıma bırakıldı
